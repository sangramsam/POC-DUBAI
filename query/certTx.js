...
var certTransaction = {
  ...
  generatePrivateKey: function (data) {
    return new Promise(function (resolve, reject) {
      const wallet = Wallet.generate();
      Registration.findOne({"_id": ObjectId(data._id)}, (err, student) => {
        let prv = wallet.getPrivateKeyString();
        student.privateKey = prv;
        student.address = wallet.getAddressString();
        student.address = wallet.getAddressString();
        student.save((err) => {
          if (err) {
            console.log('ERROR!');
            ...
            let schoolData = {
              ...
              StudentID: "STD" + Math.random().toString(36).substr(2, 9),
              Email: student.Email,
              StudentBranch: student.StudentBranch,
              Gender: student.Gender,
              DateOfBirth: student.DateOfBirth,
              EmiratesID: student.EmiratesID,
              SmartPass: student.SmartPass,
              address: student.address,
              privateKey: student.privateKey,
            };
            mailer.sendMail({privateKey: prv, email: data.Email,StudentID:schoolData.StudentID});
            resolve(schoolData);
          }
        });
      });
